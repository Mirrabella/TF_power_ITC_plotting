...
###############################################################################################
######## function prepear .h5 files for tfr ploting with power and ITC ###########
# time_bandwidth = 4 by default
def make_h5_files_power_itc(subj, r, cond, fb, data_path, L_freq, H_freq, f_step, period_start, period_end, baseline, n_cycles, time_bandwidth, freqs):
    ...
    # time_bandwidth = 4 , by default
    freq_show_baseline = mne.time_frequency.tfr_multitaper(epochs, freqs = freqs, n_cycles = n_cycles, time_bandwidth = time_bandwidth, use_fft = False, return_itc = False, average=True).crop(tmin=baseline[0], tmax=baseline[1], include_tmax=True) #frequency of baseline
    
    freq_show_baseline_mean = freq_show_baseline.data.mean(axis = -1) # averaged by time points
    ...
    epochs.resample(300) 
    # time_bandwidth = 4 , by default
    freq_show, itc = mne.time_frequency.tfr_multitaper(epochs, freqs = freqs, n_cycles = n_cycles, time_bandwidth = time_bandwidth, use_fft = False, return_itc = False, average=True)
...
#############################################################################
##################### непарный ttest Autists vs NT #######################################	
def ttest_NT_vs_A(data_path, subjects_autists, subjects_NT, parameter1, freq_range, planar, fr, n): # n - количество временных отчетов
	contr = np.zeros((len(subjects_NT), 2, 102, fr, n))

	for ind, subj in enumerate(subjects_NT):
	    #temp1 = mne.time_frequency.read_tfrs(op.join(data_path, '{0}_{1}_average_{2}_resp_{3}.h5'.format(subj, parameter1, freq_range, planar)))[0]
	    temp1 = mne.time_frequency.read_tfrs(op.join(data_path, '{0}_{1}_average_resp_{2}.h5'.format(subj, parameter1, planar)))[0]
	    
	    #temp2 = mne.time_frequency.read_tfrs(op.join(data_path, '{0}_{1}_average_{2}_resp_{3}.h5'.format(subjects_autists[ind], parameter1, freq_range, planar)))[0]
	    temp2 = mne.time_frequency.read_tfrs(op.join(data_path, '{0}_{1}_average_resp_{2}.h5'.format(subjects_autists[ind], parameter1, planar)))[0]
	...
	comp1 = contr[:, 0, :, :, :]
	comp2 = contr[:, 1, :, :, :]
	# Двухвыборочный (непарный) ттест
	
	t_stat, p_val = stats.ttest_ind(comp2, comp1, axis=0, equal_var=False)
...
def plot_deff_tf_comb_channels_without_stat(p_val, donor, mean1, mean2, folder_out, best_chan_list):
    ...
    b = donor.plot([0], baseline=None, tmin=-0.95, tmax=2.05, fmax = 31, vmin=-2.0, vmax=2.0, title='Three best sensors averaged, without stat', show = False);   
...
def plot_deff_tf(p_val, donor, mean1, mean2, folder_out, title,  treshold = 0.05): 	
    ...
    fig = donor.plot(baseline=None, mode='logratio', title=title, combine = 'mean', show = False);
    fig_stat = donor.plot(baseline=None, mode='logratio', title=title, combine = 'mean', mask = p_val.mean(axis = 0 ) < treshold, mask_style = 'contour', show = False);
    # получаем отдельную картинку для каждого канала
    for chan in range(len(donor.ch_names)):
        #name = '/home/vtretyakova/Рабочий стол/probabitily_teta/TF_plots/{0}/'.format(folder_out) + donor.ch_names[chan] + '.jpeg'
        name_stat = '/home/vtretyakova/Рабочий стол/probabitily_teta/TF_plots/{0}/'.format(folder_out) + donor.ch_names[chan] + '.jpeg'
        #b = donor.plot([chan], baseline=None, title=donor.ch_names[chan], show = False);
        b_stat = donor.plot([chan], baseline=None, title=donor.ch_names[chan], mask = p_val[chan] < treshold, mask_style = 'contour', show = False);
        #b[0].savefig(name, dpi=300, format='jpeg', transparent=False, bbox_inches=None, pad_inches=0.1, frameon=None)
        b_stat[0].savefig(name_stat, dpi=300, format='jpeg', transparent=False, bbox_inches=None, pad_inches=0.1, frameon=None)
...
def plot_tf_vs_zero_comb_channels_without_stat(p_val, donor, mean1, folder_out, best_chan_list): 	    
    ...
    b = donor.plot([0], baseline=None, tmin=-0.95, tmax=2.05, fmax = 31, vmin=-4, vmax=4, title='Three best sensors averaged, without stat', show = False);   
